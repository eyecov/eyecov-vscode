...
async function main() {
  ...
  if (
    ...
    console.error('tools/call result missing query/resolved/matchCount:', structured);
    process.exit(1);
  }

  console.log('MCP server OK: coverage_file returned query=%s resolved=%s matchCount=%s', structured.query, structured.resolved, structured.matchCount);
  proc.kill();
  process.exit(0);
}
