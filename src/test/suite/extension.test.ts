...
suite('Covflux Extension Test Suite', () => {
  ...
  test('Covflux commands should be registered', async () => {
    const showCoverage = await vscode.commands.getCommands().then((cmds) =>
      cmds.includes('covflux.showCoverage')
    );
    assert.strictEqual(showCoverage, true, 'covflux.showCoverage command should be registered');
  });
});
