...
`;
  ...
  describe("resolveFilePath", () => {
    ...
    it("resolves relative path under workspace root", () => {
      expect(
        resolveFilePath("app/Domain/Foo/Action.php", [workspaceRoot]),
      ).toBe(path.resolve(workspaceRoot, "app/Domain/Foo/Action.php"));
    });

    it("returns null when path does not exist under any root", () => {
      expect(
        resolveFilePath("app/Nonexistent.php", [workspaceRoot]),
      ).toBeNull();
    });
  });
  ...
  describe("getCandidatePathsForQuery", () => {
    ...
    it("resolves by full relative path when file exists", () => {
      const paths = getCandidatePathsForQuery("app/Domain/Foo/Action.php", [
        workspaceRoot,
      ]);
      expect(paths).toHaveLength(1);
    ...
    it("returns empty array when no match", () => {
      expect(
        getCandidatePathsForQuery("Nonexistent.php", [workspaceRoot]),
      ).toEqual([]);
    });

    it("uses custom coverageHtmlDir when provided", () => {
      const buildHtml = path.join(
        workspaceRoot,
        "build",
        "coverage-html",
        "Domain",
    ...
    it("uses sourceSegment when provided so basename search looks under src/", () => {
      ...
      const paths = getCandidatePathsForQuery("Baz.php", [workspaceRoot], {
        sourceSegment: "src",
      });
      expect(paths).toHaveLength(1);
