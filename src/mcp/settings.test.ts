...
function config(getValues: Record<string, unknown> = {}): {
...
describe("mcp/settings", () => {
  describe("isMcpServerEnabled", () => {
    ...
    it("returns false when enableMcpServer is false", () => {
      expect(isMcpServerEnabled(config({ enableMcpServer: false }))).toBe(
        false,
      );
    });
  });
  ...
  describe("isPrewarmCoverageCacheEnabled", () => {
    ...
    it("returns false when prewarmCoverageCache is false", () => {
      expect(
        isPrewarmCoverageCacheEnabled(config({ prewarmCoverageCache: false })),
      ).toBe(false);
    });
  });
});
