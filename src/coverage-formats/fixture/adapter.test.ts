...
describe('FixtureAdapter', () => {
  ...
  it('resolves both sources from multi-file fixture', async () => {
    ...
    const adapter = new FixtureAdapter({ path: 'coverage/fixture.json' });
    const recordA = await adapter.getCoverage(path.join(workspaceRoot, 'src', 'a.ts'), [workspaceRoot]);
    const recordB = await adapter.getCoverage(path.join(workspaceRoot, 'src', 'b.ts'), [workspaceRoot]);
